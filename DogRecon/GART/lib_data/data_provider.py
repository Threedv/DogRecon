...
class RealDataOptimizablePoseProviderPose(nn.Module):
    ...
    def __init__(self, dataset, balance=True, video_name=None):
        super().__init__()
        ...
        try:
            ...
            self.prob = np.load(f'/home/user/gs/huggstudy/GART/data/dog_data_official/{self.video_name}/pred/sampling_prior.npy')
            print('sample_found')
        except FileNotFoundError:
            print('sample_not_found')
            self.prob = np.ones_like(self.selection_prob) / len(self.selection_prob)
        ...
        if index is not None:
            ...
            if N == len(self.rgb_list) and not self.balance:
                ...
                if continuous and N > 1:
                    ...
                    #        prob[72] = prob[72]*2
                    #prob = prob/sum(prob)
                    print(len(self.rgb_list), N, prob.shape)
                    t = torch.from_numpy(
                        np.random.choice(len(self.rgb_list), N, p=prob)
                    ).long() # 아.. rendom cosi?
                    #print(len(self.rgb_list), N) 144, 1
                    #print(f'prob={prob},t={t}')  N= 144,len(self.rgb_list) =0 아닐까?
        if return_index:
            return t
    ...
    def prepare_for_fitting(self, dataset):
        ...
        for t in tqdm(range(len(dataset))):
            ...
            # ! white background 백그라운드 필요없지 그러니까 일단. OK>
        ...
        rgb_list = torch.stack(rgb_list)  # T, H, W, 3  #아마 배경을 모두 하안색으로 한것같다 .0~1사이의 이미지인데.. 어떻게 0번째 이미지 인줄 알 수 있을까...
        mask_list = torch.stack(mask_list)  # T, H, W
        K_list = torch.stack(K_list)  # T, 3, 3

        # balance = False니까 이거 지나간다.
        if not self.balance:
            self.selection_prob = np.ones(rgb_list.shape[0]) / rgb_list.shape[0]
            # 별거아니네 그냥 Retun함 근데 selection_prob가 뭐냐? ?? 음? 개수*1/개수? 흠... 뭐지 일단 ok
