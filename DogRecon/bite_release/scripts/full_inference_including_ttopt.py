...
def main(args):
    ...
    if cfg.data.DATASET == 'sketchfab':
        ...
        input_folder = os.path.join('/home/user/gs/huggstudy/GART/data/dog_data_official', args.image_path, 'images')
        #print(input_folder)
        ...
        for ind_img in range(input.shape[0]):
            ...
            for i in loop:
                ...
                if i % 150 == 0:    
                    ...
                    if i == 300:
                        out_path_300 = f'/home/user/gs/huggstudy/GART/data/dog_data_official/{args.image_path}/pred/{name}_comp_pred_e.png'
                        plt.imsave(out_path_300, im_masked)
                        np.savez(f'/home/user/gs/huggstudy/GART/data/dog_data_official/{args.image_path}/pred/{name}.npz',pred_betas=optimed_betas.detach().cpu().numpy(), pred_betas_limbs=optimed_betas_limbs.detach().cpu().numpy(), pred_pose=optimed_pose_with_glob.detach().cpu().numpy(), pred_trans=optimed_trans.detach().cpu().numpy(), pred_focal=optimed_camera_flength.detach().cpu().numpy())
                current_i += 1
