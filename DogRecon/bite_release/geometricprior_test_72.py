...
def main():
    ...
    for i in range(min(geo_list),max(geo_list)+1):
        ...
        degn_trans = pred_trans.copy()
        # divide 2 why -> 중점.
        degn_trans[0][0] = pred_trans[0][0] -float(torch.mean(keyp_3d[0,:24,0]).cpu().numpy())/2
        degn_trans[0][1] = pred_trans[0][1] -float(torch.mean(keyp_3d[0,:24,1]).cpu().numpy())/2
        ...
        optimizer = torch.optim.SGD([degn_trans,degn_0pose], lr=5*1e-3,momentum=0.9) #너무 빠름.
        ...
        for j in loop:
            optimizer.zero_grad()
            #pose
            smal_verts, keyp_3d, _ = smal(beta=torch.from_numpy(pred_betas).to(device), betas_limbs=torch.from_numpy(pred_betas_limbs_notail).to(device), pose=degn_0pose, trans=degn_trans, keyp_conf='olive', get_skin=True)
            #theta
            ...
            print(f'{j}_loss+reg = {loss}, degn_trans={degn_trans}')
        ...
        np.savez(f'/home/user/gs/huggstudy/GART/data/dog_data_official/{image_name}/pred/{i:04d}.npz',pred_betas=pred_betas, pred_betas_limbs=pred_betas_limbs_notail, pred_pose=degn_0pose.detach().cpu().numpy(), pred_trans=degn_trans.detach().cpu().numpy(), pred_focal=pred_focal)
