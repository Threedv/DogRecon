...
def image_preprocess(input_image, image_name, lower_contrast=True, rescale=True):
    ...
    white_back = np.ones((side_len, side_len,3), dtype=np.uint8)*255
    center = side_len // 2
    padded_image[
        center - h // 2 : center - h // 2 + h, center - w // 2 : center - w // 2 + w
    ] = image_arr[y : y + h, x : x + w]

    image_arr2 = image_arr[:,:,:3].copy()
    image_arr2[image_arr[:,:,3]<150] = [255,255,255]
    white_back[
        center - h // 2 : center - h // 2 + h, center - w // 2 : center - w // 2 + w
    ] = image_arr2[y : y + h, x : x + w]

    rgba = Image.fromarray(padded_image).resize((512, 512), Image.LANCZOS)
    rgb = Image.fromarray(white_back).resize((512,512), Image.LANCZOS)
    rgba.save(f'./oneshot_image/{image_name}_rgba.png')
    rgb.save(f'./oneshot_image/{image_name}_resize.png')
    rgba_flip = rgba.transpose(Image.FLIP_LEFT_RIGHT)
    rgb_flip = rgb.transpose(Image.FLIP_LEFT_RIGHT)
    rgba_flip.save(f'./oneshot_image/{image_name}_rgba2.png')
    rgb_flip.save(f'./oneshot_image/{image_name}_resize2.png')
