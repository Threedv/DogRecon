...
def main():
    ...
    for i in range(min(geo_list),max(geo_list)+1):
        ...
        np.savez(f'/home/user/gs/huggstudy/GART/data/dog_data_official/{image_name}/pred/{i:04d}_test.npz',pred_betas=pred_betas, pred_betas_limbs=pred_betas_limbs_notail, pred_pose=degn_0pose.detach().cpu().numpy(), pred_trans=degn_trans.detach().cpu().numpy(), pred_focal=pred_focal)
