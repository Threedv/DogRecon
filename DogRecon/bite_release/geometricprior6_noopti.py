...
def main():
    ...
    print(min(geo_list),max(geo_list)+1)
    for i in range(min(geo_list),max(geo_list)+1):
        ...
        degn_trans = torch.tensor(degn_trans, dtype=torch.float,device=device)
        ...
        np.savez(f'/home/user/gs/huggstudy/GART/data/dog_data_official/{image_name}/pred/{i:04d}.npz',pred_betas=pred_betas, pred_betas_limbs=pred_betas_limbs_notail, pred_pose=degn_pose, pred_trans=degn_trans.cpu().numpy(), pred_focal=pred_focal)
    print(min(geo_list),max(geo_list)+1)
