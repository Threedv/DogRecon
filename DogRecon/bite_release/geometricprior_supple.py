...
def barc2d_to_smal(barc2d):
    '''
    input 2d joints in coco dataset format,
    and out 2d joints in SMPL format.
    Non-overlapping joints are set to 0s. 
    '''
    assert barc2d.shape == (24, 2)
    smal = np.zeros((41, 2))
    smal[10] = barc2d[0] # 왼쪽 앞 발
    smal[9]= barc2d[1]# 왼쪽 앞 무릎
    smal[8] =barc2d[2]# 왼쪽 앞 어깨
    smal[20] =barc2d[3]# 왼쪽 뒷 발
    smal[19] =barc2d[4]# 왼쪽 뒷 무릎
    smal[18] =barc2d[5]# 왼쪽 뒷다리 어깨
    smal[14] =barc2d[6]# 오른쪽 앞 발
    smal[13] = barc2d[7]# 오른쪽 앞 무릎
    smal[12]  =barc2d[8]# 오른쪽 앞다리 어꺠 

    smal[24]  =barc2d[9]# 오른쪽 뒷 발
    smal[23]  =barc2d[10]# 오른쪽 뒷 무릎
    smal[22] =barc2d[11]# 오른쪽 뒷다리 어깨 

    smal[25] =barc2d[12]#tail start
    smal[31] =barc2d[13]#tail end

    smal[33] =barc2d[14]#left ear
    smal[34] =barc2d[15]#right ear
    smal[35] = barc2d[16]
    smal[36] =barc2d[17]
    smal[38]=barc2d[18]
    smal[37] = barc2d[19]
    ...
    smal[6]= barc2d[22] #withers
    smal[11] =barc2d[23]#thr
...
def main():
    ...
    np.save(f'/home/user/gs/huggstudy/GART/data/dog_data_official/{image_name}/pred/sampling_prior.npy',prior_npy)
    for i in range(1,72):
        #print(min(geo_list),max(geo_list)+1, i)
        #print(rot5d@rot5d == rot5d**2)
        degn_pose[0][0] =recursive_dot_product(rot5d,i)@pred_pose[0][0]
        ...
        #그냥 다 키자 일단. 그래도 안됨.
        grad_mask = torch.ones([1,35,3,3]).float().to(device)
        #
        #grad_mask[0][0] = 1
        #grad_mask[0][3] =1
        #grad_mask[0][8] = 1
        ...
        #optimizer = torch.optim.SGD([degn_trans2,degn_0pose], lr=5*1e-3,momentum=0.9) #너무 빠름. -> 이거 keyp추가하니까 nan뜬다.
        optimizer = torch.optim.SGD([degn_trans2,degn_0pose], lr=5*1e-4,momentum=0.9) #너무 느림 non뜨는게 줄긴하였으니 뜬다.. ㅋㅋ 그리고 뭔가 문제가 있음.
        ...
        target_hg_keyp = torch.tensor((np.load(f'../GART/data/dog_data_official/{image_name}/pred/{i:04d}_key.npz'))['keyp2d']).to(device)
        target_hg_keyscore = torch.tensor((np.load(f'../GART/data/dog_data_official/{image_name}/pred/{i:04d}_key.npz'))['key_score']).to(device)
        ...
        for j in loop:
            ...
            #print(f'target_hg_keyp.shape= {target_hg_keyp.shape}') #[24, 2])
            #print(f'pred_keyp_raw.shape={pred_keyp_raw.shape}') #  ([1, 25, 2])
            #print(f'target_hg_keyscore.shape={target_hg_keyscore.shape}') #([4, 24, 1])target_hg_keyscore.shape=torch.Size([24])
            #output_kp_resh = pred_keyp_raw[0,:24,:] #-target_hg_keyp
            #key_loss = ((((output_kp_resh - target_hg_keyp)[target_hg_keyscore>0]**2).sum(axis=1).sqrt() * \
            #        target_hg_keyscore[target_hg_keyscore>0])*target_hg_keyscore[target_hg_keyscore>0]).sum() / \
            #        max((target_hg_keyscore[target_hg_keyscore>0]*target_hg_keyscore[target_hg_keyscore>0]).sum(), 1e-5)
            #print(f'key_loss={key_loss}')
            ...
            #reg없애자.
            #loss += 0.01*torch.exp(torch.mean(torch.max(rot_to_rodrigues(degn_0pose[0])-limits[...,1],zeross) + torch.max(limits[...,0]-rot_to_rodrigues(degn_0pose[0]),zeross))) #너무 크다. 갑자기 loss가 inf로 쏟구침.
            #print(f'{j}_loss+reg = {torch.exp(torch.mean(torch.max(rot_to_rodrigues(degn_0pose[0])-limits[...,1],zeross) + torch.max(limits[...,0]-rot_to_rodrigues(degn_0pose[0]),zeross)))}, degn_trans2={degn_trans2}')
            ...
            #        target_hg_keyscore[target_hg_keyscore>0])*target_hg_keyscore[target_hg_keyscore>0]).sum() / \
            #        max((target_hg_keyscore[target_hg_keyscore>0]*target_hg_keyscore[target_hg_keyscore>0]).sum(), 1e-5)
            key_loss = torch.mean(torch.abs(output_kp_resh -target_hg_keyp)*target_hg_keyscore.unsqueeze(-1)) #[target_hg_keyscore>0]**2)
            print(key_loss)
            print(f'key_loss={key_loss}')    
            loss +=  0.1*key_loss       # keyloss가 너무 컸었네..  
           #loss.backward()
            #차이 확인하기.
            ...
            for point in output_kp_resh:
                cv2.circle(im_masked, tuple(point.detach().cpu().numpy().astype(np.int)), 5, (0, 0, 1), -1)
        ...
        np.savez(f'/home/user/gs/huggstudy/GART/data/dog_data_official/{image_name}/pred/{i:04d}.npz',pred_betas=pred_betas, pred_betas_limbs=pred_betas_limbs_notail, pred_pose=degn_0pose.detach().cpu().numpy(), pred_trans=degn_trans2.detach().cpu().numpy(), pred_focal=pred_focal)
