...
class AdditionalBones(nn.Module):
    def __init__(
        ...
        if self.num_bones == 0:
            #num_bones 근데 0 맞는데 이거 pose-mlp사용 안하ㄷ나?
            return
        self.mode = mode
        #pose-mlp
        ...
        if self.mode == "pose-mlp":
            ...
            for c_out in mlp_hidden_dims:
                ...
                c_in = c_out # 오 아하. 34*3 +7 -> 256->256->256
...
class GaussianTemplateModel(nn.Module):
    ...
    # 결국 가장중요한 forward 함수.  여기선 theta와 trans를 넣어서 아마 가우시안 component가 나오게 하는듯하다.
    def forward(
        ...
        # 논문에서 나오는 additional correction이 이걸 말하는거같다. 그런듯 일단 ok.
        ...
        if add_A is not None:
            ...
            T = T + add_T  # Linear t를 약간 조정하는거구나.
            additional_dict["As"] = add_A

        # Rotation과 Translation구하기 pose로 부터.
        R, t = T[:, :, :3, :3], T[:, :, :3, 3]  # B,N,3,3; B,N,3

        # mu는 그걸 rotation한 가우시안의 3d좌표를 의미하는듯.
