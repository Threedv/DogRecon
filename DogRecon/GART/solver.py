...
from viz_utils_ijcv import viz_spinning, viz_human_all, viz_dog_all
...
class TGFitter:
    def __init__(
        ...
        **kwargs,
        ...
        self.template_model_path = template_model_path #dog는 None이다. 일단 ok
        self.device = device
        ...
        self.LAMBDA_SEMANTIC = getattr(self,"LAMBDA_SEMANTIC",1)
        ...
        #
        if args.semantic == 'clip':
            clip_model =  'ViT-L/14'
            self.perceptor, preprocess = clip.load(clip_model, jit=False, download_root='clip_ckpts', device=torch.device("cuda"))
        ...
        # prepare base R
        if self.mode == "human":
            viz_base_R_opencv = np.asarray(euler2mat(np.pi, 0, 0, "sxyz")) #  ‘r’ (rotating == intrinsic), or ‘s’ (static == extrinsic).
        else:
            #dog니까 .근데  viz_base_R_opencv 가 rzyz의 차이가 뭐지? sxyz와 ? 흠.. 일단 ok
            viz_base_R_opencv = np.asarray(euler2mat(np.pi / 2.0, 0, np.pi, "rxyz")) # ‘r’ (rotating == intrinsic), or ‘s’ (static == extrinsic).
    ...
    #일단 prepare_real_seq 확인해보자. 
        ...
        ) #balance없으니까 이거 그냥 None으로 Return된다.
    ...
    def load_saved_model(self, ckpt_path=None):
        ...
        model.summary()
        return model
    #와 ;; ;미쳤네 beat만 받아서 가우시안을 update할 수 있다.
    def _get_model_optimizer(self, betas, add_bones_total_t=0):
        ...
        #W_REST_MODE는 없으니까 pose-mlp 그리고 bons 는 num_bones 0 ADD_BONES_MLP_HIDDEN_DIMS도 없음 ok,
        # num_bones가 0 이라서 add_bones 는 None이 return되는거 같은데. 허허..
        ...
        ) #아마 o번포즈 빼고 나머지 34개 + beats_limbs 7 개이다.
        #None
        #대신에 W_MEMORY_TYPE =Voxel.

        #이게 가장 중요함. 
        model = GaussianTemplateModel(
            template=template,
            add_bones=add_bones,
            #W_CORRECTION_FLAG는 True이다.
        ...
        #LR_S: 0.005
        #LR_O: 0.05
        #LR_SPH: 0.005 그냥 learning rate를 의미하는건가?
    ...
    def _get_pose_optimizer(self, data_provider, add_bones):
        ...
        if data_provider is not None:
            ...
            # pose_base_list,rest??
            # pose_base_list = 아마 0번 포즈
            # pose_rest_list= 나머지 pose를 의미한다. 
            pose_optim_l.extend(
                [
                    {
    ...
    def _guide_step(
        ...
        if head_flag:
            ...
            loss, render_pkg_list = _inner_step(guidance, T_camo, K, pose)
        return loss, render_pkg_list
    #여기가 model에서 가장 중요한 부분이라고 생각이드네.
    def _fit_step(
        self,
        model,
        data_pack,
        ...
        # model에 pose랑 trans을 넣네. 근데 beta는 안넣나? 이미 initial할때 들어갔었나?
        #결국 GART에 넣어서 가우시안 파라미터 뽑아내기.
        ...
        #print(f'len(gt_rgb)={len(gt_rgb)}') 1
        ...
        for i in range(len(gt_rgb)):
            ...
            #여기 real_gt_mask도 같이 해야함.
            real_rgb_target[real_gt_mask == 0] = bg_tensor[None, None, :]

            #결국 3D가우시안을 rendering한 2d !!! 맞다. 예상대로~
            render_pkg = render_cam_pcl(
            ...
            #여기인듯하다. use_box_crop_pad -1이니까.  근데 len(gt_rgb아마 1일거같은데.)
            else:
                rgb_target = gt_rgb[i]
            ...
            #print(f"rgb_target = {rgb_target}, rgb_target.shape = {rgb_target.shape}") #torch.Size([512, 512, 3]) 여기도 0~1사이값
            #print(f"render_pkg['rgb'] = {render_pkg['rgb']}, render_pkg['rgb'] = {render_pkg['rgb'].shape}") #render_pkg['rgb'] = torch.Size([3, 512, 512]) 여기도 0~1사이값
            
            # rgb_target -> zero123
            # mask_target -> zero123 mask (SAM)
            # render_pkg["rgb"] -> output of GART (RGB)
            # render_pkg["alpha"] -> output of GART (MASK), rotation of input SMAL mask(Rendering)
            # real_rgb_target -> real input image (one dog image) ,RGB
        
            _loss_recon = abs(render_pkg["rgb"].permute(1, 2, 0) - rgb_target).mean()  # C,H,W H,W,C ->로 바꾸는 과정같은데 permute (0,1,2)
            _loss_mask = abs(render_pkg["alpha"].squeeze(0) - mask_target).mean()
            if args.semantic == 'clip':
                #plt.imsave('./pred.png',render_pkg["rgb"].permute(1, 2, 0).detach().cpu().numpy())
                #plt.imsave('./gt.png',real_rgb_target.cpu().numpy())
                _loss_semantic = 1 - torch.cosine_similarity(self.get_clip_feature(render_pkg["rgb"].unsqueeze(0)),self.get_clip_feature(real_rgb_target.permute(2, 0,1).unsqueeze(0)))
                #print(_loss_semantic)
            if args.sampling == 'mask':
                #print(f'_loss_mask={_loss_mask}') 0과 1사이의 값이고. 이거 print(f'len(gt_rgb)={len(gt_rgb)}') 1이라 for문 의미 없긴하다.근데 학습중에 _loss_mask도 줄어들기 때문에 weight로 쓰기에 부적합할 수 있다. 하지만 경향성은 체크할 수 있을듯하다.
                #print(f'cos_sim={torch.cosine_similarity(self.get_clip_feature(real_rgb_target.permute(2, 0,1).unsqueeze(0)),self.get_clip_feature(rgb_target.permute(2, 0,1).unsqueeze(0)))}')
                cos_sim = torch.cosine_similarity(self.get_clip_feature(real_rgb_target.permute(2, 0,1).unsqueeze(0)),self.get_clip_feature(rgb_target.permute(2, 0,1).unsqueeze(0)))
                _loss_recon = _loss_recon*(1+cos_sim-float(_loss_mask.detach().cpu().numpy())) # 이렇게하면 loss_mask에도 gradient가 흐르기때문에 문제가 있음 detach로 가져오자. -> samplingloss
                #_loss_recon = _loss_recon*(1-float(_loss_mask)) # maskloss 가 더 좋네.
                #_loss_recon = _loss_recon*(1+float(_loss_semantic.detach().cpu().numpy())*0.00001 -_loss_mask  )
                #print(f'weight={(1-float(_loss_mask))}')
                #_loss_recon = _loss_recon*(1-float(_loss_mask))# 이건 전부 opti할때 사용하자.
            #수정 
            #print(f"real_rgb_target.shape ={real_rgb_target.shape}, real_rgb_target={real_rgb_target}")# torch.Size([512, 512, 3])
        ...
        if args.semantic == 'clip':
            loss_semantic = loss_semantic/ len(gt_rgb)
        
        if args.semantic == 'clip':

            loss = (
                loss_recon + self.LAMBDA_SSIM * loss_ssim + self.LAMBDA_LPIPS * loss_lpips + self.LAMBDA_SEMANTIC*loss_semantic
            )
            return (
            loss,
            loss_mask,
    ...
    def run(
        ...
        if real_flag:
            ...
            total_t = real_data_provider.total_t #그냥 길이.
        else:
        ...
        # 일단 확인. 
        #여기가 더 중요함. 내가 궁금한것 pose!
        # 확인환료했는데  pose도 optimize를 하네.
        ...
        for step in tqdm(range(self.TOTAL_steps)):
            ...
            loss = 0.0
            # N_POSES_PER_STEP 이게 뭔가 batch같은데 확인. N_POSES_PER_STEP 다 1이네.. 뭐지 ? 일단 그런거같은데.
            if real_flag:
                # real_data_pack 이게 중요할듯하다. 몇번을 불러오냐
                ...
                (
                    ...
                    model_ret,
                    loss_dict,
                #_fit_step 이 train의 핵심인듯.
                ) = self._fit_step(
                    model,
                    real_rgb_target = real_rgb_target,
                    real_gt_mask = real_gt_mask,
                ...
                ) #real_rgb_target 이미 불러오는것 이므로 상관이 없구나 그냥 부르기만 하면 되는구나.그러면 순서는 랝덤인가? 아니 내가 준거잖아 ;;
                #그렇지 모델 + rendering에서 loss measure하니까.
            ...
            if (
                ...
                #수정 
                loss = loss + self.LAMBDA_MASK * loss_mask # LAMBDA_MASK 이거 0이다 마스크 안들어감.
                
                self.add_scalar("loss_mask", loss_mask.detach(), step)
            self.add_scalar("loss", loss.detach(), step)

            # * Reg Terms\
            # 그냥 regulazation이라고 생각하자.
            reg_loss, reg_details = self.compute_reg3D(model)
            if step%100 == 0:
                print(f'step = {step},reg_loss={reg_loss}, loss={loss}') #step = 200,reg_loss=0.00017924880887734335, loss=0.05440541356801987
                #print(f'real_data_pack[0].shape= {real_data_pack[0].shape}') #real_data_pack[0].shape= torch.Size([1, 512, 512, 3])
                #print(f'real_data_pack={real_data_pack[0][0]}') #무슨 1만 들어가있냐..
            ...
            if (step + 1) % getattr(self, "VIZ_INTERVAL", 100) == 0 or step == 0:
                ...
                if self.mode == "human":
                    can_pose[0] = self.viz_base_R.to(can_pose.device) #3x3인데 can_pose의 0이 3x3인가보네. 뭐지 애매하긴하다. 아마 nx3x3이면 가능할듯. 0번 pose를 rot하는거니까.
                    can_pose = matrix_to_axis_angle(can_pose)[None]
                else:
                    can_pose[:3] = matrix_to_axis_angle(self.viz_base_R[None])[0] #[None]을 쓰는이유 0번째 dimention추가하기 위해서 1,3,3이 되는거지.  #torch.Size([3]) can_pose를 axis_angle로 어짜피 바꿔주네 아마 105개중에 앞 3개가 xyz에 대한 axisangle representation이니까.
...
def tg_fitting_eval(solver, dataset_mode, seq_name, optimized_seq):
    if dataset_mode == "people_snapshot":
        ...
        test(
            solver,
            seq_name=seq_name,
            tto_flag=True, #test original True
            tto_step=160,
            tto_decay=50,
            dataset_mode=dataset_mode,
            pose_base_lr=2e-2,
...
if __name__ == "__main__":
    ...
    args.add_argument("--fast", action="store_true") # action="store_true" 는 default가 False이다.
    ...
    args.add_argument("--semantic", type=str, default="none")
    args.add_argument("--sampling", type=str, default="none")
    ...
    if dataset_mode == "zju":
        ...
        mode = "dog"
        smpl_path = None # none 이네? 왜지 당연하지 smpl이아니니까.
    else:
    ...
    #print(data_provider, trainset) RealDataOptimizablePoseProviderPose() <dog_demo.Dataset object at 0x7fac9df68fa0>
    #print(data_provider(3), trainset.rgb_list[0].shape) # tuple, (512, 512, 3)
    #print(f"trainset.rgb_list[0] = {trainset.rgb_list[0]}") 0~1사이 값인듯
    #print(data_provider(3)[0].shape, len(trainset.rgb_list[0])) #torch.Size([3, 512, 512, 3]) 512 , 배치가 맞았네.. 그러면 len 
    #print(data_provider(1)[0].shape, len(trainset.rgb_list)) #torch.Size([1, 512, 512, 3]) 72
    #print(trainset.rgb_list[0])
    #im = trainset.rgb_list[0]
    #out_path = './test.png'
    ...
    if args.eval_only:
        ...
        logging.info("Done")
        sys.exit(0)
    #여기? 아니지 viz_only도 아니지. false store_true의 default는 False다.
    elif args.viz_only:
        #assert len(args.log_dir) > 0, "Please specify the log_dir for eval only mode"
        #assert osp.exists(log_dir), f"{log_dir} not exists!"
        data_provider.load_state_dict(
        ...
        if mode == "human":
            ...
            viz_dog_all(solver, data_provider, viz_name="viz_only", seq_name=f'{seq_name}')
        logging.info("Done")
        sys.exit(0)
    ...
    _, optimized_seq = solver.run(data_provider, real_rgb_target = torch.tensor(trainset.rgb_list[0], device=torch.device('cuda:0'), dtype=torch.float), real_gt_mask = torch.tensor(trainset.mask_list[0], device=torch.device('cuda:0')))
