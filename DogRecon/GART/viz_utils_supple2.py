...
def viz_dog_animation(model, animations, animation_names, save_path_animation_img, limb, trans, H, W, K, save_paths,seq_name, fps=200, device="cuda:0"):
    save_dir = '/home/user/gs/huggstudy/GART/gart_output3'
...
def viz_dog_all(solver, data_provider, model=None, ckpt_dir=None, viz_name="", seq_name=""):
    ...
    #final_pose_list_left = np.zeros((21, 35, 3), dtype = np.float32)
    ...
    for joint, (start_angle, end_angle) in start_end_values.items():
        ...
        '''
        if joint == 15:
            interpolated_angles1 = np.linspace(-0.15, 0.0, 11)
            interpolated_angles2 = np.linspace(0.0, 0.15, 10)
            interpolated_angles = np.concatenate((interpolated_angles1, interpolated_angles2))

        if joint == 16:
            interpolated_angles1 = np.linspace(start_angle, end_angle, 10)
            interpolated_angles2 = np.linspace(0.0, 0.12, 11)
            interpolated_angles = np.concatenate((interpolated_angles1, interpolated_angles2))
    ...
    #############################################
    n_final_pose_list_left = n_final_pose_list.copy()

    #for s in range(361):
    #    n_final_pose_list_left[s, 0, :] = [-1.57, -0.06, -2.83]

    #n_final_pose_list_left[:, 0:1, :] = [1.0267,  1.9684, -1.8357]

    n_final_pose_list_left[:, 0:1, :] = [1.3341,  1.7201, -1.1501]
    root_pose_first =  n_final_pose_list_left[0,0:1,:] # (1,1,3)
    root_pose_second = root_pose_first.squeeze(0) #(1,3)

    root_pose_second_tensor = torch.tensor(root_pose_second, dtype=torch.float)

    root_pose = axis_angle_to_matrix(root_pose_second_tensor) #(1,3,3)
    root_pose_2 = root_pose.clone() #(1,3,3)

    vector = torch.tensor([0, 0, -1], dtype=torch.float32, device=root_pose.device)
    b = root_pose @ vector #(1,3,3)
    y = torch.tensor([0, 1, 0], dtype=torch.float32, device=root_pose.device)
    rotation_matrix = rot5deg2radmatrix(b, y, 60).to(root_pose.device)

    root_pose = rotation_matrix @ root_pose #(1,3,3)

    root_pose_axis = matrix_to_axis_angle(root_pose) # (1,3)
    root_pose_final = root_pose_axis.squeeze(0) 
    print(f'root_pose_final.shape:{root_pose_final.shape}')
    print(f'root_pose_final:{root_pose_final}')

    n_final_pose_list_left[:, 0:1, :] = root_pose_final


    print(f'n_final_pose_list_left[0,0:1,:]:{n_final_pose_list_left[0,0:1,:]}')
    ####################################################
    n_final_pose_list_right = n_final_pose_list.copy()

    n_final_pose_list_right[:, 0:1, :] = [1.3341,  1.7201, -1.1501]

    vector = torch.tensor([0, 0, -1], dtype=torch.float32, device=root_pose_2.device)
    b_2 = root_pose_2 @ vector #(1,3,3)
    y_2 = torch.tensor([0, 1, 0], dtype=torch.float32, device=root_pose_2.device)
    rotation_matrix_2 = rot5deg2radmatrix(b_2, y_2, 330).to(root_pose_2.device)

    root_pose_2 = rotation_matrix_2 @ root_pose_2 #(1,3,3)

    root_pose_axis_2 = matrix_to_axis_angle(root_pose_2) # (1,3)
    root_pose_final_2 = root_pose_axis_2.squeeze(0)

    n_final_pose_list_right[:, 0:1, :] = root_pose_final_2    
    ####################################################

    #n_final_pose_list_head = n_final_pose_list.copy()
    n_final_pose_list_head = np.zeros((321, 35, 3), dtype = np.float32)

    n_final_pose_list_head[:, 0:1, :] = [1.3341,  1.7201, -1.1501]

    print(f'n_final_pose_list_head.shape:{n_final_pose_list_head.shape}') #(321,35,3)

    #joint = 16
    
    start_head_angle_1 = 0.87
    end_head_angle_1 = -0.15

    start_head_angle_2 = -0.15
    end_head_angle_2 = 0.4
    '''
    start_head_angle_1 = 0.1
    end_head_angle_1 = -0.15

    start_head_angle_2 = -0.15
    end_head_angle_2 = 0.87
    '''
    interpolated_angles_head = np.linspace(start_head_angle_1, end_head_angle_1, 171)
    interpolated_angles_head_2 = np.linspace(start_head_angle_2, end_head_angle_2, 150)
    interpolated_angles_final_head = np.concatenate((interpolated_angles_head, interpolated_angles_head_2))

    print(f'interpolated_angles_final_head.shape:{interpolated_angles_final_head.shape}') #(361,)

    

    for i in range(321):
        axis_angle_head = [0, 0, interpolated_angles_final_head[i]]
        new_head =  [0, -0.31, 0]

        n_final_pose_list_head[i, 15, :] = new_head
        n_final_pose_list_head[i, 16, :] = axis_angle_head
    ...
    animation_left = torch.from_numpy(n_final_pose_list_left).to(solver.device)
    animation_right = torch.from_numpy(n_final_pose_list_right).to(solver.device)
    animation_head = torch.from_numpy(n_final_pose_list_head).to(solver.device)
    ...
    animation_left[:, [32, 33, 34]] = pose[:, [32, 33, 34]]
    animation_right[:, [32, 33, 34]] = pose[:, [32, 33, 34]]
    animation_head[:, [32, 33, 34]] = pose[:, [32, 33, 34]]
    
    animation4[:,[0]] = pose_front_tensor[:, 0]
    print(f'pose_front_tensor[:, 0]:{pose_front_tensor[:, 0]}')
    print(f'animation3[0,0,:]={animation3[0,0,:]}]')
    #animations = [animation_head, animation_left, animation4, animation_right]
    #animations = [animation2, animation_head, animation4]  # Define animations
    animations = [animation_head, animation_right, animation4]
    ...
    #animation_names = ['animation2_drone', 'animation_head_moving','animation4_front_view']
    animation_names = ['animation_head_moving','animation_walk2_final','animation4_front_view']
    #animation_names = ['animation4_moving_head','animation_walk_left', 'animation4_walk_front', 'animation_walk_right']
    ...
    save_path4=osp.join(viz_dir, f"animation_{seq_name}_front.gif")

    save_path_left=osp.join(viz_dir, f"animation_{seq_name}_left.gif")
    save_path_right=osp.join(viz_dir, f"animation_{seq_name}_right.gif")
    save_path_walk2_final=osp.join(viz_dir, f"animation_{seq_name}_walk2_final.gif")

    save_path_head=osp.join(viz_dir, f"animation_{seq_name}_head.gif")
    ...
    #save_paths = [save_path2, save_path_head, save_path4]
    save_paths = [save_path_head, save_path_walk2_final, save_path4]
    #save_paths = [save_path_head, save_path_left, save_path4, save_path_right]
