...
def main():
    ...
    parser.add_argument('--input_frame', required=True, type=int, help='the path to the source video')
    ...
    input_frame = opt.input_frame
    one_npz_path = f'../GART/data/dog_data_official/{image_name}/pred'
    predz1 = dict(np.load(os.path.join(one_npz_path,f'{input_frame:04d}.npz')))
    ...
    for k in range(input_frame+1,input_frame+73):
        if k >72:
            i = k-73
        else:
            i = k
        ...
        optimizer = torch.optim.SGD([degn_trans], lr=5*1e-6,momentum=0.9)
