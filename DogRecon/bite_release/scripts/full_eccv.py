    ...
    for i, (input, target_dict) in iterable:
        ...
        for ind_img in range(input.shape[0]):
            ...
            with torch.no_grad():
                ...
                print(name)
                if name in ['0000','0072']:
                    pass
                else:
                    np.savez(f'/home/user/gs/huggstudy/GART/data/dog_data_official/{args.image_path}/pred/{name}_key.npz',keyp2d=target_kp_resh.cpu().numpy(),key_score=weights_resh.cpu().numpy())
                    continue 
