...
def viz_human_all(
    ...
    model_mask=None,
    viz_name="",
    export_mesh_flag=True,  # remove this from release version
):
    if model is None:
    ...
    for name, pose in to_viz.items():
        ...
        """if export_mesh_flag:
            ...
            mesh.export(f"{New_dogrecon}/mc_{name}.obj", "obj")"""
...
@torch.no_grad()
def viz_dog_all(solver, data_provider, model=None, ckpt_dir=None, viz_name="", export_mesh_flag=True):
    if model is None:
        model = solver.load_saved_model(ckpt_dir)
    model.eval()
