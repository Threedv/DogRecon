...
def main():
    ...
    #pred_pose = 0
    #print(pred_pose.shape) 1,35,3,3
...
#     prior_npy = np.array(prior_list)/sum(prior_list)
    ...
    #pred_pose = torch.from_numpy(np.load('./test_animalmotion/doggieMN5_runforwardRM.npy'))
    #gs/huggstudy/OmniMotionGPT/data/animals_smal_joints/doggieMN5_JumpdownRM.npy
    #pred_pose = torch.from_numpy(np.load('../OmniMotionGPT/data/animals_smal_joints/doggieMN5_JumpdownRM.npy'))
   # pred_pose = torch.from_numpy(np.load('../OmniMotionGPT/data/animals_smal_joints/doggieMN5_Jump.npy'))
    #pred_pose = torch.from_numpy(np.load('../OmniMotionGPT/data/animals_smal_joints/bear3EP_CrouchleftRM.npy'))
    # pred_pose = torch.from_numpy(np.load('../OmniMotionGPT/data/animals_smal_joints/doggieMN5_Sleep.npy')) # N, 35, 3
    #pred_pose = torch.from_numpy(np.load('../OmniMotionGPT/data/animals_smal_joints/doggieMN5_Sleep.npy')) # N, 35, 3
    
    pred_pose = torch.from_numpy(np.load('./test_animalmotion/doggieMN5_runforwardRM_pose.npy'))
    pred_pose =pred_pose.float()
    
    rot_x = torch.tensor([[ 1.0000000,  0.0000000,  0.0000000],[
  0.0000000,  0.0000000,  -1.0000000],[
  0.0000000, 1.0000000,  0.0000000]])
    rot__x = torch.tensor([[ 1.0000000,  0.0000000,  0.0000000],[
  0.0000000,  0.0000000,  1.0000000],[
  0.0000000, -1.0000000,  0.0000000]])
    rot__y = torch.tensor([[ 0.0000000,  0.0000000, -1.0000000],[
  0.0000000,  1.0000000,  0.0000000],[
  1.0000000,  0.0000000,  0.0000000]])
    rot__z = torch.tensor([[  0.0000000, 1.0000000,  0.0000000],[
  -1.0000000,  0.0000000,  0.0000000],[
  0.0000000,  0.0000000,  1.0000000 ]])
    rot_m = torch.tensor([[  0.0000000,  0.0000000,  1.0000000],[
  1.0000000,  0.0000000,  0.0000000],[
  0.0000000, 1.0000000,  0.0000000 ]])
    
    rot_y180 = torch.tensor([[ -1.0000000,  0.0000000,  0.0000000],[
   0.0000000,  1.0000000,  0.0000000],[
  -0.0000000,  0.0000000, -1.0000000 ]]) #어깨축 #근데 왜 카메라 기준으로 회전이 들어가지?
    
    # N seq
    for i in range(200):
        #print(min(geo_list),max(geo_list)+1, i)
        #print(rot5d@rot5d == rot5d**2)
        #degn_pose[0][0] = pred_pose[0][0]
        
        degn_pose = axis_angle_to_matrix(pred_pose)[i].unsqueeze(0) # N,35,3 -> N,35,3,3 -> 35,3,3 -> 1,35,3,3
        #degn_pose[0][0] = rot_x@rot__z @degn_pose[0][0] # root pose rotation for facial area visualization  0th pose
        degn_pose[0][0] = rot_x@rot_x@degn_pose[0][0]
        print(degn_pose.shape)
        #
        smal_verts, keyp_3d, _ = smal(beta=torch.from_numpy(pred_betas).to(device), betas_limbs=torch.from_numpy(pred_betas_limbs_notail).to(device), pose=degn_pose.to(device), trans=torch.from_numpy(pred_trans).float().to(device), keyp_conf='olive', get_skin=True) #D-SMAL
...
#         #grad_mask[0][11] = 1
        ...
        #for j in loop:
            # optimizer.zero_grad()
            #pose
            #smal_verts, keyp_3d, _ = smal(beta=torch.from_numpy(pred_betas).to(device), betas_limbs=torch.from_numpy(pred_betas_limbs_notail).to(device), pose=degn_0pose, trans=torch.from_numpy(pred_trans).to(device), keyp_conf='olive', get_skin=True)
            #theta
        ...
        pred_silh_images, pred_keyp_raw = silh_renderer(vertices=smal_verts, points=keyp_3d, faces=faces_prep, focal_lengths=torch.from_numpy(pred_focal).to(device)) #camera rendering
...
#            #loss.backward()
        ...
        out_path = f'/home/user/gs/huggstudy/bite_release/test.png'
